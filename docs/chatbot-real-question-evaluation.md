...
The remaining 15 intent-label mismatches in the full sweep often still produced useful answers (for example, a delivery answer despite a stock intent). They were not automatically counted as factual errors. Some safe refusals ask for a model even though a product category was supplied, because SQLite had no catalog; the current production catalog must be tested separately before product-specific answer quality can be claimed. Test contact links came from SQLite defaults and do not establish production contact settings. No new production request or deploy was part of this sweep at the time of writing.
