        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <h1 class="mt-1 text-2xl font-extrabold text-gray-950 sm:text-3xl">სასაჩუქრე ყუთის აწყობა</h1>
                    <p class="mt-2 max-w-2xl text-sm text-gray-600">აირჩიეთ მთავარი საჩუქარი, დაამატეთ აქსესუარები და ბარათი. პროდუქცია v1-ში მხოლოდ ადგილობრივი მარაგიდან ჩანს.</p>
                </div>
...
(function () {
                    ...
                    <div class="flex flex-1 flex-col p-4">
                        ...
                        <div class="mt-auto pt-4">
                            <span class="inline-flex w-full items-center justify-center gap-2 rounded-full ${isSelected ? 'bg-primary-600 text-white' : 'border border-slate-200 text-gray-700'} px-4 py-2 text-xs font-bold">
                                ...
                                ${isSelected ? 'Selected' : mode === 'main' ? 'Choose main gift' : 'Add to box'}
                            </span>
                        </div>
                    </div>
                </button>
    ...
    function selectedSummaryHtml(compact) {
        ...
        if (!lines.length) {
            return '<p class="rounded-xl border border-dashed border-slate-200 px-4 py-6 text-center text-sm text-gray-500">აირჩიეთ მთავარი საჩუქარი.</p>';
        }
        ...
        return lines.map((line) => `
            <div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-3">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-xs text-gray-500">${escapeHtml(line.variant?.name || '')} • ${line.role === 'main' ? 'Main' : 'Add-on'}</p>
                </div>
    ...
    function renderContent() {
        ...
        if (step === 'recipient') {
            html = `
                <div class="mb-5">
                    <h2 class="text-xl font-extrabold text-gray-950">ვისთვისაა საჩუქარი?</h2>
                    <p class="mt-1 text-sm text-gray-500">ეს ფილტრი პროდუქციის შეთავაზებებს დაალაგებს.</p>
                </div>
        ...
        if (step === 'occasion') {
            html = `
                <div class="mb-5">
                    <h2 class="text-xl font-extrabold text-gray-950">შემთხვევა და ბიუჯეტი</h2>
                    <p class="mt-1 text-sm text-gray-500">ბიუჯეტის გადაცილება მხოლოდ გაფრთხილებაა, ბლოკერი არა.</p>
                </div>
                <div class="mb-6">
                    <h3 class="mb-3 text-sm font-bold text-gray-900">Starter presets</h3>
        ...
        if (step === 'main') {
            ...
            html = `
                <div class="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h2 class="text-xl font-extrabold text-gray-950">მთავარი საჩუქარი</h2>
                        <p class="mt-1 text-sm text-gray-500">საჭიროა ზუსტად ერთი main item.</p>
                    </div>
                    <span class="text-xs font-semibold text-gray-500">${products.length} local-stock product</span>
                </div>
                ${products.length ? `<div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">${products.map((product) => productCard(product, 'main')).join('')}</div>` : '<p class="rounded-xl border border-dashed border-slate-200 p-6 text-center text-sm text-gray-500">ამ ფილტრით პროდუქტი არ მოიძებნა.</p>'}
            `;
        }
        ...
        if (step === 'addons') {
            ...
            html = `
                <div class="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h2 class="text-xl font-extrabold text-gray-950">დამატებები</h2>
                        <p class="mt-1 text-sm text-gray-500">სულ მაქსიმუმ ${config.maxItems} ფიზიკური პროდუქტი ერთ ყუთში.</p>
                    </div>
                    <span class="text-xs font-semibold text-gray-500">${selectedLines().length}/${config.maxItems} selected</span>
                </div>
                ${products.length ? `<div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">${products.map((product) => productCard(product, 'addon')).join('')}</div>` : '<p class="rounded-xl border border-dashed border-slate-200 p-6 text-center text-sm text-gray-500">ამ ფილტრით add-on არ მოიძებნა.</p>'}
            `;
        }
        ...
        if (step === 'card') {
            html = `
                <div class="mb-5">
                    <h2 class="text-xl font-extrabold text-gray-950">შეფუთვა და ბარათი</h2>
                    <p class="mt-1 text-sm text-gray-500">ბარათის ტექსტი HTML-ის გარეშე შეინახება და 300 სიმბოლომდეა.</p>
                </div>
                <div class="grid gap-6 lg:grid-cols-2">
                    <div>
                        <h3 class="mb-3 text-sm font-bold text-gray-900">Packaging</h3>
                        <div class="grid gap-3">
                            ${config.packaging.map((packaging) => `
                                <button type="button" data-package="${packaging.slug}" class="rounded-xl border p-4 text-left transition ${packaging.slug === state.packaging_slug ? 'border-primary-500 bg-primary-50 ring-2 ring-primary-100' : 'border-slate-200 bg-white hover:border-primary-200 hover:bg-primary-50/40'}">
                                    ...
                                    <span class="mt-1 block text-xs text-gray-500">Capacity: ${packaging.capacity_units} units</span>
                                </button>
                            `).join('')}
                        </div>
                    </div>
                    <div>
                        <label for="gift-message" class="mb-3 block text-sm font-bold text-gray-900">Greeting card message</label>
                        <textarea id="gift-message" maxlength="${config.messageMaxLength}" rows="8" class="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm focus:border-primary-500 focus:ring-primary-500" placeholder="მაგ: გილოცავ დაბადების დღეს...">${escapeHtml(state.message)}</textarea>
        ...
        if (step === 'review') {
            html = `
                <div class="mb-5">
                    <h2 class="text-xl font-extrabold text-gray-950">Review</h2>
                    <p class="mt-1 text-sm text-gray-500">დამატებამდე არჩევანი კიდევ ერთხელ მოწმდება stock/capacity/compatibility წესებით.</p>
                </div>
                <div class="grid gap-5 lg:grid-cols-2">
                    <div class="space-y-3">${selectedSummaryHtml(false)}</div>
                    <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <dl class="space-y-2 text-sm text-gray-700">
                            <div class="flex justify-between"><dt>Recipient</dt><dd class="font-semibold">${escapeHtml(bySlug(config.recipients, state.recipient_type)?.label || state.recipient_type)}</dd></div>
                            <div class="flex justify-between"><dt>Occasion</dt><dd class="font-semibold">${escapeHtml(bySlug(config.occasions, state.occasion)?.label || state.occasion)}</dd></div>
                            <div class="flex justify-between"><dt>Budget</dt><dd class="font-semibold">${escapeHtml(bySlug(config.budgetBands, state.budget_band)?.label || state.budget_band)}</dd></div>
                            <div class="flex justify-between"><dt>Packaging</dt><dd class="font-semibold">${escapeHtml(bySlug(config.packaging, state.packaging_slug)?.label || state.packaging_slug)}</dd></div>
                            ${state.message ? `<div class="border-t border-slate-200 pt-2"><dt class="mb-1 text-gray-500">Message</dt><dd class="rounded-xl bg-white p-3 text-gray-700">“${escapeHtml(state.message)}”</dd></div>` : ''}
                        </dl>
                    </div>
                </div>
            `;
        ...
        content.innerHTML = `
            ...
            <div class="mt-6 flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:justify-between">
                ...
                <button type="button" data-next class="inline-flex items-center justify-center gap-2 rounded-full bg-primary-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-primary-700">
                    ${state.step === steps.length - 1 ? 'Add to cart' : 'Continue'} <i class="fa-solid fa-arrow-right text-xs"></i>
                </button>
            </div>
        `;
    }
    ...
    function renderSummary() {
        ...
        const body = `
            <div class="sticky top-24 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between gap-3">
                    <h2 class="text-base font-extrabold text-gray-950">Gift summary</h2>
                ...
                <dl class="mt-4 space-y-2 text-sm text-gray-700">
                    <div class="flex justify-between"><dt>Packaging</dt><dd class="font-semibold">${escapeHtml(packaging?.label || state.packaging_slug)}</dd></div>
                    <div class="flex justify-between border-t border-slate-100 pt-2 text-base font-extrabold text-gray-950"><dt>Total</dt><dd class="text-primary-700">${money(total)}</dd></div>
                </dl>
                ...
                <button type="button" data-add-cart class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary-600 px-5 py-3 text-sm font-bold text-white hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50" ${disabled ? 'disabled' : ''}>
                    <i class="fa-solid fa-cart-plus text-xs"></i> ${state.isAdding ? 'Adding...' : 'Add gift box to cart'}
                </button>
            </div>
        `;
        ...
        mobile.innerHTML = `
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0 flex-1">
                    <p class="truncate text-xs font-semibold text-gray-500">${selectedLines().length} selected • ${escapeHtml(packaging?.label || state.packaging_slug)}</p>
                ...
                <button type="button" data-next class="inline-flex shrink-0 items-center justify-center rounded-full bg-primary-600 px-4 py-2.5 text-sm font-bold text-white">
                    ${state.step === steps.length - 1 ? 'Cart' : 'Next'}
                </button>
            </div>
        `;
    }
    ...
    async function refreshPrice() {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                state.error = errors[0] || data.message || 'Gift box cannot be priced.';
                state.priced = null;
                renderSummary();
                return;
            }
    ...
    async function addToCart() {
        ...
        try {
            ...
            if (!response.ok || !data.success) {
                const errors = data.errors ? Object.values(data.errors).flat() : [];
                state.error = errors[0] || data.message || 'Gift box could not be added.';
