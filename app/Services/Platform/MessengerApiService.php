...
class MessengerApiService
{
    ...
    {
        $this->accessToken = config('services.facebook.messenger_access_token', '');
    }
    ...
    public function sendMessage(string $recipientId, string $message, ?string $mediaUrl = null): array
    {
        if (!$this->accessToken) {
            return [
                'success' => false,
                'error' => 'messenger_access_token_missing',
            ];
        }
        ...
        try {
            ...
            if (!$response->successful()) {
                Log::warning('Messenger send message failed', [
            ...
            Log::error('Messenger send message exception', [
                'error' => $e->getMessage(),
            ]);

            return [
                'success' => false,
                'error' => 'messenger_send_exception',
    ...
    public function sendTypingIndicator(string $recipientId, bool $isTyping): void
    {
        ...
        try {
            ...
            Log::debug('Messenger typing indicator failed', [
                'error' => $e->getMessage(),
            ]);
        }
    }
    ...
    public function fetchUserProfile(string $userId): array
    {
        ...
        try {
            ...
            Log::warning('Messenger fetch user profile failed', [
                'user_id' => $userId,
                'error' => $e->getMessage(),
            ]);
